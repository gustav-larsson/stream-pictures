...
import {
  ...
  ButtonsShape,
  ButtonsLabel
} from "../types/paypal/buttons";
import { coerceBooleanProperty } from "@angular/cdk/coercion";
import { OnShippingChangeData, OnShippingChangeActions } from "../types/paypal/buttons";
import { OnApproveData, OnApproveActions } from "../types/paypal/buttons";
import { OnCancelData, OnCancelActions } from "../types/paypal/buttons";
import { OnClickData, OnClickActions } from "../types/paypal/buttons";
import { OnInitData, OnInitActions } from "../types/paypal/buttons";
import { SubscriptionRequest } from "../types/paypal/subscription";
import { OnErrorData } from "../types/paypal/buttons";
import { OrderRequest } from "../types/paypal/order";
import { Funding } from "../types/paypal/common";
import { PayPal } from "../types/paypal/paypal";
...
export interface OnApprove {
  onApprove(OnApproveData: any, OnApproveActions: any): Promise<void>;
}

export interface OnShippingChange {
  onShippingChange(
    OnShippingChangeData: any,
    OnShippingChangeActions: any
  ): Promise<void>;
}
...
/** PayPal Buttons */
@Directive({ selector: "wmPaypal" })
export class PayPalButtonsDirective implements OnChanges, OnDestroy {
  ...
  /** Emits on Click */
  @Output() emitClick = new EventEmitter<OnClickData>();
  ...
  /** Emits on Error */
  @Output() emitError = new EventEmitter<OnErrorData>();
  ...
  private buttons(): Promise<Buttons> {
    ...
    return this.paypal.then(pp =>
      pp.Buttons({
        ...
        createOrder:
          this.type === "checkout"
            ? (_, actions) => actions.order.create(this.request as OrderRequest)
            : undefined,

        // Handles subscription creation otherwise
        createSubscription:
          this.type === "subscription"
            ? (_, actions) => actions.subscription.create(this.request as SubscriptionRequest)
            : undefined,
        ...
        onError: data => this.emitError.emit(data),
